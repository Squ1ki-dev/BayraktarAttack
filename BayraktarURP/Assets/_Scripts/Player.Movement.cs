...
public partial class Player
{
    private void Move()
    {
        ...
        if (_joystick.GetForce() != 0) _rigidbody.MovePosition(_rigidbody.position + (transform.forward * _moveSpeed * Time.deltaTime)); //* ((_joystick.Horizontal + _joystick.Vertical) / 2)));
    }
}
